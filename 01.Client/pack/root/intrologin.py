...
class LoginWindow(ui.ScriptWindow):
	...
	def __LoadScript(self, fileName):
		...
		try:
			...
			# saved accs #
			self.AccUse[0]				= GetObject("Acc1_use")
			self.AccUse[1]				= GetObject("Acc2_use")
			self.AccUse[2]				= GetObject("Acc3_use")
			self.AccUse[3]				= GetObject("Acc4_use")
			self.AccUse[4]				= GetObject("Acc5_use")
			self.AccUse[5]				= GetObject("Acc6_use")
			
			self.AccDel[0]				= GetObject("Acc1_del")
			self.AccDel[1]				= GetObject("Acc2_del")
			self.AccDel[2]				= GetObject("Acc3_del")
			self.AccDel[3]				= GetObject("Acc4_del")
			self.AccDel[4]				= GetObject("Acc5_del")
			self.AccDel[5]				= GetObject("Acc6_del")
			
			self.AccName[0]				= GetObject("Acc1_text")
			self.AccName[1]				= GetObject("Acc2_text")
			self.AccName[2]				= GetObject("Acc3_text")
			self.AccName[3]				= GetObject("Acc4_text")
			self.AccName[4]				= GetObject("Acc5_text")
			self.AccName[5]				= GetObject("Acc6_text")
			
			for i in range(6):
				if self.AccName[i]:
					self.AccName[i].SetFontColor(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
			
			self.serverSelectButton.SetTextColorRGB(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
			self.serverExitButton.SetTextColorRGB(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
			self.loginButton.SetTextColorRGB(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
			self.loginExitButton.SetTextColorRGB(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
			self.LoginSaveButton.SetTextColorRGB(215.0 / 255.0, 206.0 / 255.0, 115.0 / 255.0)
	...
	def __OnClickLoginSaveButton(self):
		...
		if slot == None:
			self.PopupNotifyMessage("Es ist kein Slot zum speichern frei!") #translate_me
			return
			
		fd = open("loginsettings/loginsetting" + str(slot) + ".cfg", "r+")
		fd.write (user +"\n")
		fd.write (pwd)
		fd.close()
	...
	def __LoadACCNames(self):
		for i in range(6):
			...
			if cfgRead != "":
				self.AccName[i].SetText(str(cfgRead))
				self.AccUse[i].Show()
				self.AccDel[i].Show()
			else:
